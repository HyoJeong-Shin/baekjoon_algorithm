...
def hanoi(n,a,b,c): 
    if n == 1:
        print(a,c)
    else:   # n개의 원판이 있을 때, n-1개의 원판 즉, 맨 밑의 원판을 제외하고 
        hanoi(n-1, a, c, b) # 1번에서 2번으로 옮긴 뒤, 
        print(a,c)  # 맨 밑의 원판을 1번에서 3번으로 옮김
        hanoi(n-1, b, a, c) # 그리고 n-1개의 원판들을 다시 2번에서 3번으로 옮김

sum = 1
for i in range(n-1):
    sum = sum * 2 + 1
print(sum)
hanoi(n, 1, 2, 3)   #n개의 원판, 1번 장대, 2번 장대, 3번 장대